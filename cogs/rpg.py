...
class RPG(commands.Cog):
    ...
    async def status(self, interaction: discord.Interaction):
        ...
        embed.set_footer(text=f"距離考試還有 {min_days} 天，加油！")
    ...
    async def change_job(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="🏰 轉職大廳", 
            description=f"恭喜 **{u['name']}** 達到 Lv.5！\n請選擇你的修練道路，這將決定你的戰鬥風格與屬性成長：", 
            color=0x00ff00
        )
        embed.add_field(name="📐 微積分大師 (Intellect)", value="• **定位**: 玻璃大砲、法術爆發\n• **成長**: `INT+4`, `VIT+1`", inline=False)
        embed.add_field(name="🍎 物理大師 (Strength)", value="• **定位**: 重裝戰士、物理輸出\n• **成長**: `STR+4`, `VIT+1`", inline=False)
        embed.add_field(name="💻 計概大師 (Balanced)", value="• **定位**: 全能型、穩定輸出\n• **成長**: `STR+2`, `INT+2`, `VIT+1`", inline=False)
        embed.add_field(name="📖 英文大師 (Luck)", value="• **定位**: 爆擊流、賭徒風格\n• **成長**: `LUK+4`, `STR+1`", inline=False)
        embed.set_footer(text="⚠️ 注意：職業一旦選擇後無法變更！")
            ...
            try:
                ...
                if cmd_type == "speak":
                    ...
                    if target_ch:
                        ...
                        await message.channel.send("❌ 找不到該頻道", delete_after=5)
                    return
